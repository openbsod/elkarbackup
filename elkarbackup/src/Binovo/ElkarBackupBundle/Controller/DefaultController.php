         ...
         if (is_dir($realPath)) {
             if ('download' == $action) {
                 ...
                 $f = function() use ($realPath){
                     $command = sprintf('cd "%s"; /usr/local/bin/sudo /usr/bin/tar cfz - "%s"', dirname($realPath), basename($realPath));
                     passthru($command);
                 };
                ...
                $f = function() use ($realPath){
                    $command = sprintf('cd "%s"; /usr/local/bin/sudo /usr/local/bin/zip -r - "%s"', dirname($realpath), basename($realpath));
                    passthru($command);
                };
                $this->info('Download backup directory %clientid%, %jobid% %path%',
                            array('%clientid%' => $idClient,
